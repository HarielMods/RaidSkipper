...
local ADDON_NAME, RaidSkipper = ...
RaidSkipper.debug = false
RaidSkipper.DBVersion = 200 -- removing db due to account wide function
...
local PLAYER_DIFFICULTY1 = PLAYER_DIFFICULTY1 -- Normal
local PLAYER_DIFFICULTY2 = PLAYER_DIFFICULTY2 -- Heroic
local PLAYER_DIFFICULTY6 = PLAYER_DIFFICULTY6 -- Mythic
...
function DB.init()
    -- Database of saved characters is no longer needed
    raid_skipper_db = nil
...
-- RaidSkipper:   Vault of the Incarnates: Completed
-- RaidSkipper:   Aberrus, the Shadowed Crucible: Completed
-- RaidSkipper:   Amirdrassil, the Dream's Hope: Completed
-- RaidSkipper: War Within
-- RaidSkipper:   Nerub-ar Palace: Completed
-- RaidSkipper:   Liberation of Undermine: Completed

-- [Completed|Incomplete|In Progress] [Difficulty]

-- Reduce this entire addon to this function...
...
local commands = {
    ["help"] = PrintHelp,
    -- ["list"] = showMySkips, -- legacy: not needed with account wide function
    -- ["show"] = CreateWindow, -- Show UI window
}
...
function SlashHandler(args)
    ...
    local cmd = RaidSkipper.GetArg(args, 1):lower()
    if commands[cmd] then
        commands[cmd](args)
    elseif expansionAbbreviations[cmd] then
        local expansionData = GetExpansionDataFromName(expansionAbbreviations[cmd])
        if expansionData then
            ShowExpansion(expansionData)
        else
            RaidSkipper:Print("Unknown expansion: " .. cmd)
        end
    else
        RaidSkipper:Print("Unknown command: " .. cmd)
    end
end
